...
import { CategoriaValidationPipe } from './pipes/categorias-validation.pipe';
...
export class CategoriasController extends BaseController<categoriasEntity>{
    ...
    async getAllCategorias(@Req() req: RequestWithUser) {
        ...
        @Post()
        @Action('agregar')
        async createCategoria(@Body(CategoriaValidationPipe) categoriaData: Partial<categoriasEntity>, @Req() req: RequestWithUser) {
            const user = req.user;
        ...
        @Put(':id')
        @Action('modificar')
        async updateCategoria(@Param('id') id: number, @Body(CategoriaValidationPipe) categoriaData: Partial<categoriasEntity>, @Req() req: RequestWithUser) {
            const user = req.user;
            ...
            if (user.empresa?.id) {
                ...
                if (existingCategoria.empresa_id !== user.empresa.id) {
                    throw new BadRequestException('No tienes permisos para modificar esta categoría');
                }
        ...
        async deleteCategoria(@Param('id') id: number, @Req() req: RequestWithUser) {
            ...
            if (user.empresa?.id) {
                const existingCategoria = await this.categoriasService.findById(id);
                if (existingCategoria.empresa_id !== user.empresa.id) {
                    throw new BadRequestException('No tienes permisos para eliminar esta categoría');
                }
            }
        ...
        async bulkUpdateCategoriaStatus(@Body() body: { ids: number[], estado: boolean }, @Req() req: RequestWithUser) {
            ...
            try {
                ...
                throw new BadRequestException(`Error al actualizar categorías: ${error.message}`);
            }
        }
